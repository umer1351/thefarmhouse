...
function themetechmount_get_option( $option, $inner_option='' ){
	...
	if( !is_array($presentup_theme_options) ){
		...
		if( is_multisite() ){
			$presentup_theme_options = get_site_option('presentup_theme_options');
		} else {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line posted-on"><i class="fa fa-calendar"></i> <span class="screen-reader-text tm-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="tm-meta-line comments-link"><i class="tm-presentup-icon-comment-1"></i> ';
						ob_start();
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="fa fa-tag"></i> <span class="screen-reader-text tm-hide">%1$s </span>%2$s</span>',
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="tm-team-list-title"><i class="tm-presentup-icon-phone"></i> '. esc_attr__('Phone','presentup') .' :</div>
						<div class="tm-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="tm-team-list-title"><i class="tm-presentup-icon-mail"></i> '. esc_attr__('Email','presentup') .' :</div>
				...
				case 'website':
					...
					$return = '<div class="tm-team-list-title"><i class="tm-presentup-icon-world"></i> '. esc_attr__('Website','presentup') .' :</div>
...
function themetechmount_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_kw_presentup = ( !empty($data['library_kw_presentup']) ) ? $data['library_kw_presentup'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			...
			$return = do_shortcode('[tm-icon type="'.$library.'" icon_kw_presentup="'.$library_kw_presentup.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function themetechmount_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'presentup' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Next post:', 'presentup' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'presentup' ) . '</span> ' .
				'<span class="screen-reader-text tm-hide">' . esc_attr__( 'Previous post:', 'presentup' ) . '</span> ' .
				'<span class="post-title tm-hide">%title</span>',
		) );
...
function themetechmount_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'presentup'), $val['pf_details_line_title'] ); }
...
function themetechmount_portfoliobox_media_link( $default_icon='tm-presentup-icon-search', $video_icon='tm-presentup-icon-video', $audio_icon='tm-presentup-icon-music-alt', $slider_icon='tm-presentup-icon-gallery-1' ){
	...
	$icon_code='<i class="tm-presentup-icon-plus-1"></i>';
	...
	// default output
	$return = '<a class="tm_prettyphoto" title="' . the_title_attribute( 'echo=0' ) . '" href="' . esc_url(themetechmount_portfolio_single_image_path()) . '">' . $icon_code . '</a>';
...
function themetechmount_gallerystyle_portfoliobox_media_link( $default_icon='tm-presentup-icon-search', $video_icon='tm-presentup-icon-video', $audio_icon='tm-presentup-icon-music-alt', $slider_icon='tm-presentup-icon-gallery-1' ){
	...
	// default output
	$return = '<a class="tm_prettyphoto" data-gal="prettyPhoto[gal]" title="' . the_title_attribute('echo=0') . '" href="' . esc_url(themetechmount_portfolio_single_image_path()) . '">' . $icon_code . '</a>';
...
function themetechmount_portfolio_description( $post_id='' ){
	...
	// preparing final output
	$return = '<div class="tm-portfolio-description">' . $box_title . '' . $content . '</div><!-- .tm-portfolio-description -->';
...
/**
* Global function - This will return array of Testimonial templates
*/
...
function themetechmount_global_testimonial_template_list( $for_vc=false ){
	$return = array(
		'top-image' => esc_attr__('Testimonial Style 1', 'presentup'),
		'style-1' => esc_attr__('Testimonial Style 2', 'presentup'),
	);
	if( $for_vc==true ){ // for vc
		$return = array_flip( $return );
	}
...
function themetechmount_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"topimage-bottomcontent" => esc_attr__("Top Image Bottom Content", "presentup"),
		"left-image"  => esc_attr__("Left Image and Right Content", "presentup"),
	);
...
function themetechmount_box_wrapper( $position='start', $cptname='blog', $vars=array() ){
	...
	if( $position=='start' ){
		...
		$classArray[] = 'themetechmount-boxes-sortablebutton-'. $sortable_buttontype;
...
function themetechmount_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		$title             = sprintf( esc_attr__( '%s ', 'presentup' ), $error404_big_text );
...
function themetechmount_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="tm-header-icon tm-header-wc-cart-link"><a href="' . $cart_url . '"><i class="tm-presentup-icon-basket"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="tm-header-icon tm-header-search-link"><a href="#"><i class="tm-presentup-icon-search"></i></a></div>';
	}
...
function themetechmount_header_style_class( $echo=false ){
	...
	$total_count	= 0;
	if ( has_nav_menu( 'tm-main-menu' ) ) {  // if menu is set
		$menu_locations = get_nav_menu_locations();
		$menu			= wp_get_nav_menu_object( $menu_locations[ 'tm-main-menu' ] );
...
function themetechmount_fbar_close_icon(){
	...
	$return = '<span class="tm-fbar-close-icon tm-icolor-' . themetechmount_sanitize_html_classes( themetechmount_get_option('fbar_icon_color_close') ) . '" style="display:none;">' . themetechmount_create_icon_from_data( $fbar_handler_icon_close, true ) . '</span>';
...
function themetechmount_fbar_close_icon_for_content_area(){
	...
	$fbar_handler_icon_close = themetechmount_get_option('fbar_handler_icon_close');
	$return = '<span class="tm-fbar-close-icon tm-icolor-' . themetechmount_sanitize_html_classes( themetechmount_get_option('fbar_icon_color_close') ) . '">' . themetechmount_create_icon_from_data( $fbar_handler_icon_close, true ) . '</span>';
...
function themetechmount_footer_row_class( $row='first' ){
	...
	// border type
	if( !empty($presentup_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' tm-bordertype-'.sanitize_html_class($presentup_theme_options[$row.'_footer_bordertype']);
	}
	
	//widget border color
	if( !empty($presentup_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' tm-widgetbordercolor-'.sanitize_html_class($presentup_theme_options[$row.'_footer_widget_bordercolor']);
	}
...
if( !function_exists('themetechmount_box_team_appointment_btn') ){
function themetechmount_box_team_appointment_btn(){
	$return = '';
	$btn_text = '';
	$btn_link = '';
	...
	$meta_data = get_post_meta( $post_id, 'themetechmount_team_member_details', true );
	if( !empty($meta_data['tm_team_info']['team_details_line_btn_text']) ){
		$btn_text = $meta_data['tm_team_info']['team_details_line_btn_text'];
		$btn_link = $meta_data['tm_team_info']['team_details_line_btn_link'];
	}
	...
	if( !empty($btn_text) && !empty($btn_link) ){
		...
		<div class="tm-team-member-appointment-btn-wrapper">
			<div class="vc_btn3-container vc_btn3-left">
				<a class="tm-vc_general tm-vc_btn3 tm-vc_btn3-size-sm tm-vc_btn3-shape-square tm-vc_btn3-style-flat tm-vc_btn3-weight-no tm-vc_btn3-color-inverse" href="'. esc_url($btn_link) .'" title="'. esc_attr($btn_text) .'" target="_self">'. esc_attr($btn_text) .'</a>
...
if( !function_exists('themetechmount_text_abovemenu') ){
function themetechmount_text_abovemenu(){
	$return		 = '';
	$headerstyle = themetechmount_get_headerstyle();
	$header_text_abovemenu = themetechmount_get_option('header_text_abovemenu');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text_abovemenu ) ){
		$header_text_abovemenu = themetechmount_wp_kses( $header_text_abovemenu );
		$return      = '<div class="tm-classicheader-text-area"><div class="tm-header-text">'.do_shortcode( $header_text_abovemenu ).'</div></div>';
	}
...
if( !function_exists('themetechmount_text_belowmenu') ){
function themetechmount_text_belowmenu(){
	$return		 = '';
	$headerstyle = themetechmount_get_headerstyle();
	$header_text_belowmenu = themetechmount_get_option('header_text_belowmenu');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text_belowmenu ) ){
		$header_text_belowmenu = themetechmount_wp_kses( $header_text_belowmenu );
		$return      = '<div class="tm-header-text-area">'.do_shortcode( $header_text_belowmenu ).'</div>';
	}
...
function themetechmount_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'presentup') . '';
...
function themetechmount_event_readmore(){ 
	$return   = '';
	$readMore = esc_attr__('See Details', 'presentup') . ' <i class="kwicon-fa-angle-right"></i>';
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment-box">
			<div class="comment-meta commentmetadata">
				<?php printf( '<cite class="tm-comment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'presentup' ) . '</span>', get_comment_author_link() ); ?>
...
function themetechmount_search_results_box_title( $post_type='post' ){
	...
	if( !empty($singular_name) ){
		...
		$return .= '<div class="tm-sresults-title-w"><h2 class="tm-sresults-title">' . sprintf(
				esc_attr__('%s results','presentup'),
...
function themetechmount_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = esc_attr_x( '%1$s on %2$s', '1: post format name. 2: date', 'presentup' );
	} else {
		$format_prefix = '%2$s';
	}
...
if ( ! function_exists( 'presentup_blogstyle2_meta' ) ) :

function presentup_blogstyle2_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="tm-meta-line tags-links"><i class="fa fa-tag"></i> <span class="screen-reader-text tm-hide" tm-hide>%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'presentup' ),
							$tags_list
						);
					}
...
if ( ! function_exists( 'presentup_posttime_meta' ) ) :
function presentup_posttime_meta( $metafor="blogbox" ) {
